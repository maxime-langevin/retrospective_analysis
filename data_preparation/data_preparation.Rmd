...
This notebook prepares the data used in our analyses. It also produces the figures plotting the raw comparisons between reality and scenarios displayed in our article.
...
#when scenarios and reality are not  perfectly align even before the report publication date, function to add vertical or horizontal offset
...
We cannot apply the same method as ICU beads for new hospital admissions, since the modellers use a modified indicator not reported in data.gouv (see details in [Paireau et al 2022](https://www.pnas.org/doi/abs/10.1073/pnas.2103302119)). So for the period after July 2021, we have to extract the true data of "new hospital admissions" from the different subsequent reports, when it is reported.
...
```{r}
rm(x_reality_offset, x_scenarios_offset, y_reality_offset, y_scenarios_offset, day_offset)
```
...
```{r graph_all_ICU}
#plot
g1 <- ggplot(data_ICU_beds) +
   #dates of reports on x axis
  geom_rug(data = report_dates_ICU, aes(report)) +
  #dates of reports on x axis
  scale_x_continuous(
  ...
  geom_hline(yintercept = max_ICU_beds, linetype="dashed") +
  annotate(
    'text', x = as.Date("2021-04-15"), y = max_ICU_beds, label = "1st wave peak", 
    color = "black", fontface = "italic", family = "Times New Roman", vjust=-.4
    ) + 
  #Ile de France region graph limits
  annotate(
    "rect",
    xmin = as.Date("2020-03-01"), xmax = as.Date("2020-08-01"), ymin = 0, ymax = 3300,
    fill=alpha("white", .95), color=alpha("black", .5)
    ) +
  #IDF scenarios in front of white rectangle
  geom_line(
    data = data_ICU_beds %>% filter(report=="Apr 28, 2020"),
    aes(date, value, group=interaction(scenario_type, report), color=abs(error)), 
    linewidth=1.5
    ) +
  #reality just in Ile-de-France region
  geom_line(
    data=reality_ICU_beds_IDF %>% filter(date<as.Date("2020-07-01")), 
    aes(date, ICU_beds)) +
  #1st wave peak in Ile-de-France region
  annotate(
    "segment", 
    y = max_ICU_beds_IDF, yend = max_ICU_beds_IDF, x = as.Date("2020-03-15"), xend = as.Date("2020-07-15"),
    linetype="dashed"
    ) +
  annotate(
    'text', x = as.Date("2020-08-01"), y = 3300, label = " IdF region", 
    color = "black", fontface = "italic", family = "Times New Roman", vjust=-0.2, hjust=1, size=3.2
    ) + 
  labs(
...
#small offset to position summer 2021 reports
report_dates_hosp$place[report_dates_hosp$place=="2021-07-26"] <- report_dates_hosp$place[report_dates_hosp$place=="2021-07-26"]-10
report_dates_hosp$place[report_dates_hosp$place=="2021-08-05"] <- report_dates_hosp$place[report_dates_hosp$place=="2021-08-05"]+3
...
report_dates_hosp$place[report_dates_hosp$place=="2021-02-02"] <- report_dates_hosp$place[report_dates_hosp$place=="2021-02-02"]-5
report_dates_hosp$place[report_dates_hosp$place=="2021-02-08"] <- report_dates_hosp$place[report_dates_hosp$place=="2021-02-08"]+10
...
#plot
g2 <- ggplot(data_new_hosp) +
   #dates of reports on x axis
  geom_rug(data = report_dates_hosp, aes(report)) +
  #dates of reports on x axis
  scale_x_continuous(
  ...
  annotate(
    'text', x = as.Date("2020-04-15"), y = max_new_hosp, label = "1st wave peak", 
...
g1_b <- g1 +
  ...
  labs(x="", subtitle = "Intensive Care Units beds", y="") 
g2_b <- g2 +
  theme(legend.position = "none") +
  labs(x="publication dates of the reports", subtitle = "New Hospital Admissions", y="") 

plot_grid(
  plot_grid(
    g1_b, g2_b, labels = c("a", "b"), nrow=2
  ),
  p,
