...
We were unable to justify a clear choice between a 14-day and a 10-day ICU length of stay (Figures 5 and 6). Ultimately, we retained only the 10-day scenarios (Figure 6), as they are more favorable to the modelers.
...
temp <- f_prepare_to_save(
  scenarios %>%
    select(
      date, reality,
      # keep only 10-day ICU length of stay
      blue_full_10, blue_dashed_10, purple_full_10, purple_dashed_10, pink_full_10, pink_dashed_10 
      ),  
  reality_ICU_beds,  "ICU_beds"
  )
...
We were unable to justify a clear choice between the 10, 14 or 17-day ICU length of stay. Ultimately, we retained only the 10-day scenarios, as they are more favorable to the modelers.
...
temp <- f_prepare_to_save(
  scenarios %>% select(
    date, reality,  
    # keep only 10-day ICU length of stay
    RedFull, BlueFull, PurpleFull, GreenFull
    ),
  reality_ICU_beds,  "ICU_beds")
...
f_save_graph_pdf_png(
  "../graphs/raw_comparisons/example_extraction",
  5, 4, dpi_resolution
)
```
...
data_new_hosp <- bind_rows(lapply(
  c("2021_02_08", "2021_02_14", "2021_02_23", "2021_04_26", "2021_05_21", "2021_07_26", "2021_08_05", "2021_10_04", "2022_01_07"), 
  f_read_new_hosp
  ))
...
without colored errors

```{r}
# --- ICU: same figure but without colored errors ---
# - reality in red
# - scenarios in grey (slightly darker)
# - no error legend

# pane of reality data only for 1st wave to show peak
g1_first_wave_grey <- ggplot(reality_ICU_beds %>% filter(date < date_first_wave_max)) +
  geom_line(
    aes(date, ICU_beds),
    color = "#ff0000"
  ) +
  geom_hline(
    yintercept = max_ICU_beds,
    linetype = "dashed"
  ) +
  annotate(
    "text",
    x = as.Date("2020-03-05"), y = max_ICU_beds,
    label = "1st wave peak\nin April 2020",
    color = "black", fontface = "italic", family = "Times New Roman",
    vjust = -.2, hjust = 0
  ) +
  theme(
    legend.position = "none",
...
# pane of scenarios vs reality (no error colors)
g1_scenarios_grey <- ggplot(data_ICU_beds) +
  # dates of reports on x axis
  geom_rug(
    data = report_dates_ICU,
    aes(report)
  ) +
  scale_x_continuous(
    breaks = report_dates_ICU$place,
  ...
  # scenarios curves in grey (darker than before)
  geom_line(
    aes(date, value, group = interaction(scenario_type, report)),
    color = "#B0B0B0",
    linewidth = 1.5
  ) +
  # reality curve in red
  geom_line(
    data = reality_ICU_beds %>% filter(date > date_scenarios_min - 15),
    aes(date, ICU_beds),
    color = "#ff0000"
  ...
  coord_cartesian(
    expand = FALSE,
    xlim = c(date_scenarios_min - 15, NA),
    ylim = c(0, NA)
  ) +
  labs(
    x = element_blank(),
...
# combination of the 2 panes
g1_grey <- g1_first_wave_grey + g1_scenarios_grey +
  plot_layout(widths = c(first_wave_rel_duration, scenarios_rel_duration)) &
  ylim(0, 12500)
...
```{r}
# --- New hospitalizations: same figure but without colored errors ---
# - reality in red
# - scenarios in grey (slightly darker)
# - no error legend

# plot 1st wave (reality only)
g2_first_wave_grey <- ggplot(reality_new_hosp_adm %>% filter(date < date_first_wave_max)) +
  # smoothed reality in red
  geom_line(
    aes(date, new_hosp_smooth),
    color = "#ff0000"
  ) +
  # unsmoothed reality in red (lighter via alpha)
  geom_line(
    aes(date, new_hosp),
    color = "#ff0000", alpha = .2
  ) +
  # peak line
  geom_hline(
    yintercept = max_new_hosp,
    linetype = "dashed"
  ) +
  theme(
    legend.position = "none",
  ...
  annotate(
    "text",
    x = as.Date("2020-03-05"), y = max_new_hosp,
    label = "1st wave peak\nin April 2020",
    color = "black", fontface = "italic", family = "Times New Roman",
    vjust = -.2, hjust = 0
  )

# plot scenarios vs reality (no error colors)
g2_scenarios_grey <- ggplot(data_new_hosp) +
  # dates of reports on x axis
  geom_rug(
    data = report_dates_hosp,
    aes(report)
  ) +
  scale_x_continuous(
    breaks = report_dates_hosp$place,
...
g2_grey <- g2_first_wave_grey + g2_scenarios_grey +
  plot_layout(widths = c(first_wave_rel_duration, scenarios_rel_duration)) &
  ylim(0, 5500)
...
```{r}
# --- Combine ICU + New Hosp (no error colors) WITH a legend (reality vs scenarios) ---
# Assumes you already created:
#   g1_first_wave_grey, g1_scenarios_grey
#   g2_first_wave_grey, g2_scenarios_grey

label_scenarios <- "\nScenarios\ncomparable\nto reality"

# 1) Add a legend on ONE panel only (here: g2_scenarios_grey), and remove elsewhere
cols_legend <- setNames(
  c("#ff0000", "#B0B0B0"),
  c("Reality", label_scenarios)
)

g1_scenarios_grey <- g1_scenarios_grey + theme(legend.position = "none")

g2_scenarios_grey_leg <- g2_scenarios_grey +
  aes(color = NULL) +
  # Map colors to labels via invisible mapping (keeps geometry unchanged)
  geom_line(
    data = data_new_hosp %>% slice(1),
    aes(date, value, color = label_scenarios),
    alpha = 0
  ) +
  geom_line(
    data = reality_new_hosp_adm %>% slice(1),
    aes(date, new_hosp_smooth, color = "Reality"),
    alpha = 0
  ) +
  scale_color_manual(
    values = cols_legend,
    breaks = c("Reality", label_scenarios),
    name = ""
  ) +
  guides(
...
# Extract legend, then remove legend from plot panel
leg <- get_legend(g2_scenarios_grey_leg)
g2_scenarios_grey_noleg <- g2_scenarios_grey + theme(legend.position = "none")

# 2) Build the two combined rows (ICU / hosp)
g1_b_grey <-
  g1_first_wave_grey + labs(y = element_blank()) +
  g1_scenarios_grey + labs(x = element_blank()) +
  plot_annotation(subtitle = "Intensive Care Units beds") +
  plot_layout(widths = c(first_wave_rel_duration, scenarios_rel_duration)) &
  ylim(0, 12500)

g2_b_grey <-
  g2_first_wave_grey + labs(y = element_blank()) +
  g2_scenarios_grey_noleg + labs(x = "publication dates of the reports") +
  plot_annotation(subtitle = "New Hospital Admissions") +
  plot_layout(widths = c(first_wave_rel_duration, scenarios_rel_duration)) &
  ylim(0, 5500)

# 3) Combine with legend column
plot_grid(
  plot_grid(g1_b_grey, g2_b_grey, nrow = 2),
  leg,
  ncol = 2,
  rel_widths = c(.85, .15)
)
...
# save
f_save_graph_pdf_png(
  "../graphs/raw_comparisons/all_scenarios_reality_no_error_colors",
  8, 6, dpi_resolution
)
...
temp <- bind_rows(
  ...
  read_csv("source_data/improper_comparisons/improper_comparison_Feb_08_2021_ICU.csv") %>%
...
f_graph_short_term_changes <- 
  function(
    ...
    g <- ggplot(temp2) +
      ...
      scale_color_manual(values=c('#ff0000','#B0B0B0')) +
      labs(
